...
bool ff_osd_kbd_active = false; // ScrLk toggle: keyboard controls Gotek
...
void ff_osd_update()
{
...
#ifdef KBD_ENABLE
    // Check cross-core Gotek toggle request (from keyboard ScrLk)
    // ScrLk toggles keyboard control of Gotek without sending any button codes.
    if (osd_gotek_request)
    {
        osd_gotek_request = false;
    ...
    if (ff_osd_kbd_active)
    {
        ...
        if (held & OSD_VIRT_UP)    buttons |= FF_OSD_BUTTON_LEFT;
        if (held & OSD_VIRT_DOWN)  buttons |= FF_OSD_BUTTON_RIGHT;
        if (held & OSD_VIRT_SEL)   buttons |= FF_OSD_BUTTON_SELECT;
    }
#endif
