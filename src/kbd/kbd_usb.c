/**
 * kbd_usb.c - USB HID keyboard driver using TinyUSB Host
 *
...
static const uint8_t hid_to_universal[] = {
    ...
    KEY_ENTER, KEY_ESC, KEY_BACK_SPACE, KEY_TAB, KEY_SPACE,
    // 0x2D-0x31: Minus, Equal, LeftBracket, RightBracket, Backslash
    KEY_MINUS, KEY_EQUALS, KEY_LEFT_BR, KEY_RIGHT_BR, KEY_BACKSLASH,
    // 0x32: Europe1 (non-US #)
    NO_KEY,
    ...
    KEY_PRT_SCR, KEY_SCROLL_LOCK, KEY_PAUSE_BREAK,
    ...
    KEY_NUM_SLASH, KEY_NUM_MULT, KEY_NUM_MINUS, KEY_NUM_PLUS,
    KEY_NUM_ENTER,
...
#define HID_TO_UNIVERSAL_SIZE (sizeof(hid_to_universal) / sizeof(hid_to_universal[0]))

static kb_state_t kb_state_usb;
static usb_event_fn usb_event_cb = NULL;
...
static void process_kbd_report(uint8_t const *report, uint16_t len)
{
    ...
    if (modifier & KEYBOARD_MODIFIER_LEFTCTRL)    SET_STATE_KEY(new_state, KEY_L_CTRL);
    if (modifier & KEYBOARD_MODIFIER_LEFTSHIFT)   SET_STATE_KEY(new_state, KEY_L_SHIFT);
    if (modifier & KEYBOARD_MODIFIER_LEFTALT)     SET_STATE_KEY(new_state, KEY_L_ALT);
    if (modifier & KEYBOARD_MODIFIER_LEFTGUI)     SET_STATE_KEY(new_state, KEY_L_WIN);
    if (modifier & KEYBOARD_MODIFIER_RIGHTCTRL)   SET_STATE_KEY(new_state, KEY_R_CTRL);
    if (modifier & KEYBOARD_MODIFIER_RIGHTSHIFT)  SET_STATE_KEY(new_state, KEY_R_SHIFT);
    if (modifier & KEYBOARD_MODIFIER_RIGHTALT)    SET_STATE_KEY(new_state, KEY_R_ALT);
    if (modifier & KEYBOARD_MODIFIER_RIGHTGUI)    SET_STATE_KEY(new_state, KEY_R_WIN);

    // Map key array (up to 6 simultaneous keys)
    for (int i = 2; i < 8; i++) {
        uint8_t hid_code = report[i];
        if (hid_code == 0 || hid_code >= HID_TO_UNIVERSAL_SIZE)
            continue;
        uint8_t ucode = hid_to_universal[hid_code];
        if (ucode != NO_KEY)
            SET_STATE_KEY(new_state, ucode);
    }

    // Check if state actually changed
    if (memcmp(&kb_state_usb, &new_state, sizeof(kb_state_t)) == 0)
        return;

    kb_state_usb = new_state;
...
void tuh_hid_umount_cb(uint8_t dev_addr, uint8_t idx)
{
    if (dev_addr == usb_dev_addr && idx == usb_hid_idx) {
        ...
        usb_dev_addr = 0;
        usb_hid_idx = 0;
        memset(&kb_state_usb, 0, sizeof(kb_state_usb));
