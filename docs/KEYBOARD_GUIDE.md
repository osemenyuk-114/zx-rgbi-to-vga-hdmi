...
| Key        | Action                                            |
|------------|---------------------------------------------------|
| **PrtScr** | Open / toggle OSD setup menu                      |
...
| Key       | Action                              |
|-----------|-------------------------------------|
| **ScrLk** | Toggle Gotek keyboard mode (on/off) |
...
- Press **ScrLk** again to deactivate and release the keyboard.
...
- Mapping table: `kbd_zx.c` (`zx_key_map[]` with fast LUT for O(1) lookup).
- ZX output: CH446Q analog switch IC, delta-apply (only changed switches sent).
- OSD bridge: `kbd_osd.c` — Core 1 intercepts hotkeys, Core 0 injects virtual buttons.
- USB host: `kbd_usb.c` — `hid_to_universal[]` lookup table, boot protocol reports.
- PS/2 driver: `kbd_ps2.c` — PIO state machine, IRQ on RX FIFO, scancode Set 2.
